/* === ARQUIVO app.js (CORREÇÃO DE TIMING - DOMContentLoaded) === */

// --- VERIFICAÇÃO DE SEGURANÇA (EXECUTADA IMEDIATAMENTE) ---
// Isso precisa ser verificado antes de tudo, pois o DOM não é necessário.
if (typeof moduleContent === 'undefined' || typeof moduleCategories === 'undefined' || typeof questionSources === 'undefined') {
    
    // Esconde o conteúdo o mais rápido possível
    document.documentElement.innerHTML = `
    <head>
        <title>Erro Crítico</title>
        <style>
            body { font-family: sans-serif; background-color: #fef2f2; color: #991b1b; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
            .error-box { max-width: 600px; padding: 2rem; border: 2px solid #fecaca; background: #fff; border-radius: 1rem; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
            h2 { font-size: 1.75rem; font-weight: bold; margin-bottom: 1rem; }
            p { font-size: 1.1rem; line-height: 1.6; margin-bottom: 1.5rem; }
            code { background: #fee2e2; padding: 2px 6px; border-radius: 4px; font-family: monospace; }
        </style>
    </head>
    <body>
        <div class="error-box">
            <h2>Erro Crítico de Carregamento</h2>
            <p>Um arquivo de dados essencial (<code>data.js</code> ou <code>course.js</code>) não foi encontrado ou está corrompido.</p>
            <p>Verifique se todos os arquivos estão na pasta correta e se as variáveis em <code>data.js</code> estão sendo expostas globalmente (<code>window.moduleContent = ...</code>).</p>
        </div>
    </body>`;
    
    console.error("Erro FATAL: Arquivo data.js ou course.js não carregado ou incompleto (moduleContent, moduleCategories, ou questionSources ausente).");
    
    // Interrompe a execução de todo o script
    throw new Error("Dependências de dados ausentes. O script não pode continuar.");
}
// --- FIM DA VERIFICAÇÃO DE SEGURANÇA ---


// --- DEFINIÇÃO DE TODAS AS FUNÇÕES E VARIÁVEIS ---
// (Estas variáveis precisam ser acessíveis por onLoginSuccess)

let contentArea;
let totalModules;
let completedModules;
let notifiedAchievements;
let currentModuleId;
let cachedQuestionBanks;

let toastContainer, sidebar, sidebarOverlay, printWatermark, achievementModal, achievementOverlay;
let closeAchButton, breadcrumbContainer, loadingSpinner, resetModal, resetOverlay;
let confirmResetButton, cancelResetButton;


// --- FUNÇÃO PRINCIPAL DE INICIALIZAÇÃO (A SER CHAMADA PELO DOM) ---
function init() {
    // --- Seletores do DOM (Agora seguros para chamar) ---
    contentArea = document.getElementById('content-area');
    totalModules = Object.keys(moduleContent).length;
    completedModules = JSON.parse(localStorage.getItem('gateBombeiroCompletedModules_v3')) || [];
    notifiedAchievements = JSON.parse(localStorage.getItem('gateBombeiroNotifiedAchievements_v3')) || [];
    currentModuleId = null;
    cachedQuestionBanks = {};

    toastContainer = document.getElementById('toast-container');
    sidebar = document.getElementById('off-canvas-sidebar');
    sidebarOverlay = document.getElementById('sidebar-overlay');
    printWatermark = document.getElementById('print-watermark');
    achievementModal = document.getElementById('achievement-modal');
    achievementOverlay = document.getElementById('achievement-modal-overlay');
    closeAchButton = document.getElementById('close-ach-modal');
    breadcrumbContainer = document.getElementById('breadcrumb-container');
    loadingSpinner = document.getElementById('loading-spinner');
    resetModal = document.getElementById('reset-modal');
    resetOverlay = document.getElementById('reset-modal-overlay');
...
// Agora, o init() SÓ é chamado depois que o DOM está 100% carregado.
document.addEventListener('DOMContentLoaded', () => {
    // Chama a inicialização principal do app
    init();
    
    // Configura os listeners de UI que estavam soltos
    setupHeaderScroll();
    setupRippleEffects();
});
